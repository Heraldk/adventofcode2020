...
class Day04 : AoCDay {
    func readPassportData( rowNum : Int, rows : [String] ) -> (Int, [String : String]) {
        var rowsRead = 0
        var passportData = [String : String]()
        
        while( rowNum + rowsRead < rows.count && rows[rowNum+rowsRead] != "" ) {
            let data = rows[rowNum+rowsRead].components(separatedBy: CharacterSet(charactersIn: " :") )
            
            for i in stride(from: 0, to: data.count, by: 2) {
                passportData[data[i]] = data[i+1]
            }
        ...
        return (rowsRead, passportData)
    }
    
    let requiredFields = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
    
    func validatePassportFields( passport : [String: String]) -> Bool {
        if passport.count < 7 || passport.count > 8 {
            return false // needs at least 7 fields and not more than 8
        }
        // check that it has the 7 required fields (skipping cip)
    ...
    func validateNumberInRange( val : String, low : Int, high : Int ) -> Bool {
    ...
    func validateHeight( value : String ) -> Bool {
        ...
        let index = value.index(value.startIndex, offsetBy: value.count - 3)
        let prefix = String(value.prefix(through: index))
        
        return validateNumberInRange(val: prefix, low: min, high: max)
    }
    
    func validatePassportData( passport : [String: String]) -> Bool {
        for (key, value) in passport {
            switch key {
                case "byr":
                    if !validateNumberInRange(val: value, low: 1920, high: 2002) {
                        return false
                    }
                case "iyr":
                    if !validateNumberInRange(val: value, low: 2010, high: 2020) {
                        return false
                    }
                case "eyr":
                    if !validateNumberInRange(val: value, low: 2020, high: 2030) {
                        return false
                    }
                case "hgt":
                    if !validateHeight(value: value) {
                        return false
                    }
                case "hcl":
                    ...
                    for char in value.suffix(from: index) {
                        if !"abcdef0123456789".contains(char) {
                            return false
                        }
                    }
                case "ecl":
                    if !validEyeColours.contains(value) {
                        return false
                    }
                case "pid":
                    if value.count != 9 {
    ...
    override func problem01()
    {
        ...
        var rowNum = 0
        while rowNum < rows.count {
            let (rowsRead, passport) = readPassportData( rowNum: rowNum, rows: rows )
            rowNum += rowsRead
        ...
        for passport in passports {
            if validatePassportFields(passport: passport) {
                validCount += 1
                
                if( validatePassportData(passport: passport) ) {
                    validData += 1
                }
            }
        }
