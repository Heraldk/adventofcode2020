...
class Day07 : AoCDay {
    ...
    func parseBagRecipe( fromString: String ) -> (String, [String : Int]) {
        let vals = fromString.components( separatedBy: " " )
        let name = "\(vals[0]) \(vals[1])" // names in the input always contain qualifier and colour
        var holds = [String : Int]()
        var index = 4
        if( vals[index] != "no" ) {
            while index < vals.count { 
                holds["\(vals[index+1]) \(vals[index+2])"] = Int(vals[index])!
                index += 4
            }
        }
        return( name, holds )
    ...
    func readInput( inputStr : String ) -> [String : [String : Int]] {
        var bags = [String: [String: Int]]()
        let rows = inputStr.components(separatedBy: "\n")
        for row in rows {
            let (name, holds) = parseBagRecipe(fromString: row)
            bags[name] = holds
        }
        return bags
    }
    
    func findBagsThatCanHold( bag: String, bags : [String : [String : Int]] ) -> Int {
        var couldContain = [String: Bool]()
        var toExplore = [bag]
        while toExplore.count > 0 {
            let currentBagName = toExplore.removeLast()
            
            for (candidateBagName, candidateBagHolds) in bags {
                if candidateBagHolds[currentBagName] != nil && couldContain[candidateBagName] == nil {
                    couldContain[candidateBagName] = true
                    toExplore.append( candidateBagName )
                }
            }
        }
        
        return couldContain.count
    }
    
    override func problem01()
    {
        let bags = readInput( inputStr: input )
        print( findBagsThatCanHold( bag: "shiny gold", bags: bags ) )
    }
    
    func countContainedBags( bagName: String, bags : [String : [String : Int]] ) -> Int {
        var count = 0
        
        if let currentBag = bags[bagName] {
            for (holdBagName, bagCount) in currentBag {
                count += bagCount + bagCount * countContainedBags( bagName: holdBagName, bags: bags )
            }
        }
    ...
    {
        let bags = readInput( inputStr: input )
        print( countContainedBags( bagName: "shiny gold", bags: bags ) )
    }
