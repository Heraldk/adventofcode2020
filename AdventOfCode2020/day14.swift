...
class Day14 : AoCDay {
    ...
    func readMask( _ maskStr: String ) -> (Int, Int) {
        var andMask = 0
        var orMask = 0
        for char in maskStr {
            andMask <<= 1
    ...
    override func problem01()
    {
        ...
        for row in rows {
            let rowVals = row.components(separatedBy: CharacterSet(charactersIn: " []=") )
            if rowVals[0] == "mask" {
                (andMask, orMask) = readMask( rowVals[3] )
            } else {
                let memLoc = Int(rowVals[1])!
                let intVal = Int(rowVals[5])!
                mem[memLoc] = (intVal & andMask) | orMask
            }
        }
        
        let sum = mem.reduce(into: 0, {
            $0 += $1.value
        })
        print( sum )
    }
    
    func readMaskPartTwo( _ maskStr : String ) -> [(Int,Int)] {
        var masks = [(0,0)]
        
        for char in maskStr {
            var nextMasks = [(Int,Int)]()
            for (andMask, orMask) in masks {
                switch char {
                    case "1":
                        nextMasks.append( (andMask << 1, (orMask << 1) | 1) )
                    case "0":
                        nextMasks.append( ((andMask << 1) | 1, orMask << 1) )
                    case "X":
                        nextMasks.append( (andMask << 1, (orMask << 1) | 1) )
                        nextMasks.append( (andMask << 1, orMask << 1) )
                    default:
                        break
                }
            }
    ...
    override func problem02()
    {
        ...
        for row in rows {
            let rowVals = row.components(separatedBy: CharacterSet(charactersIn: " []=") )
            if rowVals[0] == "mask" {
                masks = readMaskPartTwo( rowVals[3] )
            } else {
                let memLoc = Int(rowVals[1])!
                let intVal = Int(rowVals[5])!
                for (andMask, orMask) in masks {
                    let maskedMemLoc = (memLoc & andMask) | orMask 
                    mem[maskedMemLoc] = intVal
                }
            }
        }
