...
class Day02 : AoCDay {
    ...
    func readInput( inputStr : String ) -> [(Policy, String)] {
        var result = [(Policy,String)]()
        for inputRow in inputStr.components(separatedBy: "\n") {
            let rowTokens = inputRow.components(separatedBy: CharacterSet( charactersIn: " -") )
            let nextPolicy = Policy(lower: Int(rowTokens[0])!, upper: Int(rowTokens[1])!, char: rowTokens[2].first!)
            result.append( (nextPolicy, rowTokens[3]))
        }
        return result
    } 
    ...
    override func problem01()
    {
        ...
        for instance in readInput( inputStr: input ) {
            var charCount = 0
            for char in instance.1 {
                if char == instance.0.char {
                    charCount += 1
                }
            }
            if charCount >= instance.0.lower, charCount <= instance.0.upper {
                validPasswords += 1
            }
        }
        print( "Part 1 valid passwords: \(validPasswords)" )
    }
    ...
    {   
        ...
        for instance in readInput( inputStr: input ) {
            // swift handling of strings is pretty annoying since they need to handle all kinds of encodings.
            // so this isn't particularly efficient, but it'll do
            let firstCharIndex = instance.1.index(instance.1.startIndex, offsetBy: instance.0.lower - 1)
            let secondCharIndex = instance.1.index(instance.1.startIndex, offsetBy: instance.0.upper - 1)
            
            let firstCharEquals = instance.1[firstCharIndex] == instance.0.char
            let secondCharEquals = instance.1[secondCharIndex] == instance.0.char
            
            if (firstCharEquals || secondCharEquals), !(firstCharEquals && secondCharEquals) {
                validPasswords += 1
            } 
        }
        print( "Part 2 valid passwords: \(validPasswords)" )
    }
