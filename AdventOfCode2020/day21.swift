...
class Day21 : AoCDay {
    ...
    struct Food {
        let ingred : Set<String>
        let allergens : Set<String>
        
        static func readFromLine( _ str : String ) -> Food {
            let firstSplit = str.components(separatedBy: CharacterSet( charactersIn: "()"))
            var ingred = Set<String>()
            var allergens = Set<String>()
            for item in firstSplit[0].components(separatedBy: " ") {
                if item.count > 0 {
                    ingred.insert(item)
                }
            }
            for aller in firstSplit[1].components(separatedBy: CharacterSet( charactersIn: ", ") ) {
                if( aller.count > 0 && aller != "contains" ) {
                    allergens.insert( aller )
                }
            }   
            return Food(ingred: ingred, allergens: allergens)
        }
    }
    
    func readInput( str: String ) -> [Food] {
        var food = [Food]()
        for row in str.components(separatedBy: "\n") {
            food.append( Food.readFromLine( row ) )
        }
        return food
    }
    ...
    {
        let food = readInput(str: input)
        let allAllergens = food.reduce(into: Set<String>(), {
            $0.formUnion( $1.allergens )
        })
        
        var allergenToPossibles = [String : Set<String>]()
        for allergen in allAllergens {
            for foodItem in food {
                if foodItem.allergens.contains( allergen ) {
                    if allergenToPossibles[allergen] == nil {
                        allergenToPossibles[allergen] = foodItem.ingred
                    } else {
                        allergenToPossibles[allergen]!.formIntersection(foodItem.ingred)
                    }
                }
            }
        }
        print( allergenToPossibles )
        let couldBeAllergens = allergenToPossibles.reduce(into: Set<String>(), {
            $0.formUnion( $1.value )
        })
        var count = 0
        for foodItem in food {
            for ingred in foodItem.ingred {
                if !couldBeAllergens.contains(ingred) {
                    count += 1
                }
            }
        }
        ...
        var allergenToIngred = [String : String]()
        var ingredToAllergen = [String : String]()
        while( couldBeAllergens.count > allergenToIngred.count ) {
            for (allergen, possibles) in allergenToPossibles {
                if( possibles.count == 1 && allergenToIngred[allergen] == nil ) {
                    allergenToIngred[allergen] = possibles.first!
                    ingredToAllergen[possibles.first!] = allergen
                } else {
                    allergenToPossibles[allergen]!.subtract(allergenToIngred.values)
                }
            }
        }
        print( allergenToIngred )
        
        let sortedIngreds = ingredToAllergen.keys.sorted(by: {
            ingredToAllergen[$0]! < ingredToAllergen[$1]!
        })
        for (index,item) in sortedIngreds.enumerated() {
            print( item, terminator: index+1 < sortedIngreds.count ? "," : "\n" )
        }
    }
