...
class Day06 : AoCDay {
    override func problem01()
    {
        ...
        let seenItems = rows.reduce( into: [[Character: Int]]( arrayLiteral: ["N" : 0]), {
            if $1 == "" {
                $0.append( ["N" : 0] ) // all characters in input are lowercase, use capital N to count number of people in each group
            } else {
                for char in $1 {
                    $0[$0.count - 1][char] = ($0[$0.count-1][char] ?? 0) + 1
                }
                $0[$0.count - 1]["N"]! += 1
            }
        })
        
        let sum = seenItems.reduce( into: (0,0), { result, param in 
            result.0 += (param.count - 1)
            result.1 += param.filter { $0.value == param["N"]! }.count - 1 // returns one more since it'll count "N" in this total, so subtract 1
        })
        print(sum)
    }
